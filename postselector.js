...
function update() {
    ...
    // update
    posts.classed("current", function (d) {
        console.log("classed " + d.id);
        return d.id == currentSelection || d.id == dragSelection
    });
    ...
    trans.selectAll('image')
        .attr("width", function (d) {
            return d.id == currentSelection ? 196 : 46;
        })
        .attr("height", function (d) {
            return d.id == currentSelection ? 196 : 46;
        });
    ...
    var nposts = posts.enter().append("g")
        ...
        .attr("transform", function (d, i) {
            return "translate(" + (16 + 333 * d.lane) + ",2000)";
        });
    nposts.transition().duration(1000)
        .attr("transform", function (d, i) {
            return "translate(" + (16 + 333 * d.lane) + "," + (10 + d.rank * 60) + ")";
        });
    nposts.append("clipPath").attr("id", function (d) {
            return "post" + d.id + "-clip"
        })
    ...
    nposts.append("rect")
        ...
        .attr("ry", 20).attr("stroke", "#f8f8f8");
    nposts.append("text")
        .classed("post", true).attr("width", 300).attr("x", 150).attr("text-anchor", "middle")
        .attr("y", 0)
        .attr("dy", "1em")
        .attr("clip-path", function (d) {
            return "url(#post" + d.id + "-clip)"
        })
        .text(function (d) {
            return d.title;
        });
    nposts.append("image").attr("xlink:href", function (d) {
        return d.iconurl ? d.iconurl : ''
            ...
            success: function (res) {
                ...
                if (res == '0' || res == '1' || res.substring(0, 1) == '#') {
                    if (res.substring(1, 2) == '#')
                        ...
                        alert("Sorry, could not save data to wordpress" + res);
                }
                $('#' + id).prop('disabled', false);
