...
function postselector_custom_box($post)
{
    ...
    <select name="postselector_output_app" id="postselector_output_app_id">
        ...
        foreach ($apps as $app) {
            if (current_user_can('edit_post', $app->ID)) {
                ...
                ?>
                <option
                    value="<?php echo $app->ID ?>" <?php echo $selected ?> ><?php echo esc_html($app->post_title) ?></option>
            <?php }
        }
        ?>    </select><br/>
    <?php
    ...
    <label><input type="hidden" name="postselector_use_union_shown" value="1"/>
        <input type="checkbox" name="postselector_use_union" <?php echo $postselector_use_union ? 'checked' : '' ?> />Share
        selection via Union server:</label><br/>
    <input type="text" name="postselector_union_url" value="<?php esc_attr($postselector_union_url) ?>"
    ...
    <option
        value="<?php echo STATUS_MODE_NONE ?>" <?php if (STATUS_MODE_NONE == $postselector_status_mode) echo 'selected' ?>>&mdash;
        None &mdash;</option>
    <option
        value="<?php echo STATUS_MODE_PUBLISH ?>" <?php if (STATUS_MODE_PUBLISH == $postselector_status_mode) echo 'selected' ?>>
        Publish or trash
    </option>
</select><br/>
    <label for="postselector_vote_duration">Vote Valid Until</label><br/>
    <select name="postselector_duration_day" id="postselector_duration_day_id">
        <?php
        for ($d = 0; $d < 31; $d++) {
            ?>
            <option
    ...
    <select name="postselector_duration_hour" id="postselector_duration_hour_id">
        <?php
        for ($h = 0; $h < 24; $h++) {
            ?>
            <option
                value="<?php echo $h ?>"> <?php echo $h ?> </option>
            <?php
        }
        ?>
    </select>Hours
    <select name="postselector_duration_min" id="postselector_duration_min_id">
        <?php
        for ($m = 1; $m < 60; $m++) {
            ?>
            <option
...
function postselector_save_postdata($post_id)
{
    ...
    $time_out = current_time("timestamp");
    if (array_key_exists('postselector_duration_day', $_POST)) {
        $time_out += intval($_POST['postselector_duration_day']) * 86400;
    }
    if (array_key_exists('postselector_duration_hour', $_POST)) {
        $time_out += intval($_POST['postselector_duration_hour']) * 3600;
    }
    if (array_key_exists('postselector_duration_min', $_POST)) {
        $time_out += intval($_POST['postselector_duration_min']) * 60;
    }
    update_post_meta($post_id,
        '_postselector_vote_duration', $time_out);
    $voted = array();
    update_post_meta($post_id,
        '_postselector_vote_list', $voted);
}
            ...
            if (null !== $post) {
                ...
                $result = mysqli_query($connection, $sql);
                 if (mysqli_num_rows($result) <= 0) {
                    $sql = "INSERT INTO wp_vote (ID, Yes, NA, No) VALUES ('$id',1,0,0)";
    ...
    foreach ($choices['rejected'] as $pid) {
        if (!current_user_can('delete_post', $pid)) {
            ...
            if (null !== $post) {
                ...
                $result = mysqli_query($connection, $sql);
                if (mysqli_num_rows($result) <= 0) {
                    $sql = "INSERT INTO wp_vote (ID, Yes, NA, No) VALUES ('$id',0,0,1)";
                    mysqli_query($connection, $sql);
                    } else {
...
function vote_save()
{
    ...
    if ($postselector_input_category) {
        ...
        foreach ($ps as $p) {
            if (current_user_can('read_post', $p->ID)) {
                $id = $p->ID;
                $sql = "SELECT ID, Yes, No FROM wp_vote WHERE ID = '$id'";
                $result = mysqli_query($connection, $sql);
                if (mysqli_num_rows($result) > 0) {
                    while ($row = mysqli_fetch_assoc($result)) {
                        if ($row["Yes"] > $row["No"]) {
...
if (is_admin()) {
    ...
    add_action('wp_ajax_vote_save', 'vote_save');
}